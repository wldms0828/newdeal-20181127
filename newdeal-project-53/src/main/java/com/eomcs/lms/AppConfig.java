...
// Mybatis의 DAO 구현체 자동 생성하는 도우미를 추가한다.
// =>인터페이스가 들어있는 패키지를 지정한다.
// 자동 생성된 DAO 구현체가  SQL을 찾을 때 인터페이스의 전체 이름으로 찾는다.
// 따라서 SQL mapper 파일의 namespace 이름은 인터페이스의 전체 이름과 같아야 한다.
// 인터페이스의 메소드 이름 ,파라미터와 리턴타입이 
// SQL 매퍼의 이름, 파라미터 타입,리턴 타입과 일치해야 한다.
